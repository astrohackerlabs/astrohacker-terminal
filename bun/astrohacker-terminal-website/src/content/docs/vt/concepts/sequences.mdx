...
| `ESC X`  | `0x98` | SOS  | Start of String             | Obsolete. Ignored by Astrohacker Terminal.                                                                           |
| `ESC ^`  | `0x9e` | PM   | Private Message             | Obsolete. Ignored by Astrohacker Terminal.                                                                           |
...
non-numeric identifiers before the semicolon, but Astrohacker Terminal does not support these.

> [!NOTE]
> Due to yet more convoluted historical reasons, OSC sequences in xterm (and
> by extension Astrohacker Terminal) may be terminated with the Bell character (`BEL`,
> `0x07`) instead of the standard String Terminator (`ST`, `ESC \` or `0x9c`).
>
> While they are in most cases interchangeable, Astrohacker Terminal will try to echo back
...
An example OSC sequence is `OSC 2 ; 👻  Astrohacker Terminal 👻  ST` (where `ST` is
...
Astrohacker Terminal supports APC sequences, most notably the Kitty Graphics Protocol
...
SOS and PM sequences are entirely ignored by Astrohacker Terminal.
