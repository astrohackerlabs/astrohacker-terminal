...
Astrohacker Terminal attempts to differentiate between the two by trying to find the
...
questionable security-wise at worst, Astrohacker Terminal only *implements* the extensions
that are listed below. There are many more that Astrohacker Terminal does parse, but either
due to security reasons or the fact that Astrohacker Terminal does not try to be a ConEmu
...
error, or indeterminate, which is represented visually in Astrohacker Terminal as a progress
...
> cleaning up its progress state. This is why Astrohacker Terminal has a **hardcoded
