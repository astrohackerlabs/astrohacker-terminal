...
> [!NOTE]
> Astrohacker Terminal only recognizes `c`, `p`, and `s` as valid values in `t`.
> Other types are treated as aliases for `c`. When `t` is omitted,
> Astrohacker Terminal defaults to querying or modifying the standard clipboard
> (`c`).

> [!WARNING]
> Astrohacker Terminal currently only allows specifying one clipboard per sequence.
