...
const DATA_URL: &str = "data:text/html,%3Ctitle%3EGirlbat%20Engine%3C/title%3E%3Ca%20href=%22https://example.com/girlbat-engine%22%20style=%22position:fixed;left:0;top:0;width:80px;height:80px;display:block%22%3Eengine%3C/a%3E%3Cscript%3Econsole.log(%22Girlbat%20console%20engine%22)%3C/script%3E";
const DIALOG_DATA_URL: &str = "data:text/html,%3Ctitle%3EGirlbat%20Dialog%3C/title%3E%3Cscript%3Econst%20r%20%3D%20prompt%28%22girlbat-dialog-message%22%2C%20%22girlbat-dialog-default%22%29%3B%20console.log%28%22girlbat-dialog-result%3A%22%20%2B%20r%29%3B%3C/script%3E";
...
impl EngineService {
    pub fn start() -> Result<Self, String> {
        ...
        match ready_rx.recv_timeout(Duration::from_secs(30)) {
            Ok(Event::RuntimeCreated { owner_thread }) => {
                eprintln!("[Girlbat] engine owner started thread={owner_thread}");
                Ok(Self {
                    tx: command_tx,
    ...
    pub fn shutdown(&self) {
        ...
        if let Some(join) = self.join.lock().unwrap().take() {
            if join.join().is_err() {
                eprintln!("[Girlbat] engine owner thread panicked during shutdown");
            }
        }
    }
}
...
pub fn owner_thread_smoke() -> bool {
    ...
    eprintln!("[Girlbat] engine-thread-smoke caller_thread={caller_thread}");
    ...
    match recv_event(&event_rx, "runtime created") {
        Some(Event::RuntimeCreated { owner_thread }) => {
            eprintln!("[Girlbat] engine-thread-smoke owner_thread={owner_thread}");
            if owner_thread == caller_thread {
                eprintln!("[Girlbat] engine-thread-smoke owner thread matched caller thread");
                ok = false;
            }
        }
        Some(Event::Failed { message }) => {
            eprintln!("[Girlbat] engine-thread-smoke runtime failed: {message}");
            ok = false;
        }
        other => {
            eprintln!("[Girlbat] engine-thread-smoke unexpected startup event: {other:?}");
            ok = false;
        }
    }
        ...
        if ok {
            match empty_active_rx.recv_timeout(Duration::from_secs(5)) {
                Ok(Ok(outcome)) if outcome.affected_count == 0 && outcome.snapshots.is_empty() => {
                    eprintln!("[Girlbat] engine-thread-smoke empty gui-active broadcast ok");
                }
                other => {
                    eprintln!(
                        "[Girlbat] engine-thread-smoke empty gui-active broadcast failed: {other:?}"
                    );
                    ok = false;
                }
            }
        ...
        if ok {
            ...
            if ok {
                match rx.recv_timeout(Duration::from_secs(5)) {
                    ...
                    other => {
                        eprintln!("[Girlbat] engine-thread-smoke create reply failed: {other:?}");
                        ok = false;
                        None
                    }
                }
    ...
    if let Some(tab_id) = created_tab_id {
        match recv_event(&event_rx, "view created") {
            Some(Event::ViewCreated {
                ...
                eprintln!("[Girlbat] engine-thread-smoke view created tab_id={event_tab_id}");
            }
            Some(Event::Failed { message }) => {
                eprintln!("[Girlbat] engine-thread-smoke create failed: {message}");
                ok = false;
            }
            other => {
                eprintln!("[Girlbat] engine-thread-smoke unexpected create event: {other:?}");
                ok = false;
            }
        }
    }
    ...
    if let Some(tab_id) = created_tab_id {
        match recv_event(&event_rx, "load finished") {
            Some(Event::LoadFinished {
                ...
                eprintln!(
                    "[Girlbat] engine-thread-smoke load finished tab_id={event_tab_id} url={url}"
                );
            }
            Some(Event::Failed { message }) => {
                eprintln!("[Girlbat] engine-thread-smoke load failed: {message}");
                ok = false;
            }
            other => {
                eprintln!("[Girlbat] engine-thread-smoke unexpected load event: {other:?}");
                ok = false;
            }
        }
    }
        ...
        if ok {
            match resize_rx.recv_timeout(Duration::from_secs(5)) {
                Ok(Ok(tab)) if tab.width == 1024 && tab.height == 768 => {
                    eprintln!(
                        "[Girlbat] engine-thread-smoke resized tab_id={tab_id} size={}x{}",
                        tab.width, tab.height
                    );
                }
                other => {
                    eprintln!("[Girlbat] engine-thread-smoke resize reply failed: {other:?}");
                    ok = false;
                }
            }
        }
        ...
        if ok {
            match focus_rx.recv_timeout(Duration::from_secs(5)) {
                Ok(Ok(tab)) if tab.focused => {
                    eprintln!("[Girlbat] engine-thread-smoke focused tab_id={tab_id}");
                }
                other => {
                    eprintln!("[Girlbat] engine-thread-smoke focus reply failed: {other:?}");
                    ok = false;
                }
            }
        }
        ...
        if ok {
            match color_rx.recv_timeout(Duration::from_secs(5)) {
                Ok(Ok(tab)) if !tab.dark => {
                    eprintln!(
                        "[Girlbat] engine-thread-smoke color-scheme tab_id={tab_id} dark=false"
                    );
                }
                other => {
                    eprintln!("[Girlbat] engine-thread-smoke color-scheme reply failed: {other:?}");
                    ok = false;
                }
            }
        }
        ...
        if ok {
            match inactive_rx.recv_timeout(Duration::from_secs(5)) {
                Ok(Ok(outcome)) if outcome.affected_count == 1 => {
                    eprintln!(
                        "[Girlbat] engine-thread-smoke gui-active broadcast active=false target_count={}",
                        outcome.affected_count
                    );
                }
                other => {
                    eprintln!(
                        "[Girlbat] engine-thread-smoke gui-active broadcast failed: {other:?}"
                    );
                    ok = false;
                }
            }
        ...
        if ok {
            ...
            if ok {
                match active_rx.recv_timeout(Duration::from_secs(5)) {
                    ...
                    {
                        eprintln!(
                            "[Girlbat] engine-thread-smoke gui-active tab_id={tab_id} active=true"
                        );
                    }
                    other => {
                        eprintln!(
                            "[Girlbat] engine-thread-smoke gui-active reply failed: {other:?}"
                        );
                        ok = false;
                    }
                }
        ...
        for (label, command) in input_commands {
            ...
            if ok {
                match rx.recv_timeout(Duration::from_secs(5)) {
                    Ok(Ok(tab)) => {
                        eprintln!(
                            "[Girlbat] engine-thread-smoke input {label} tab_id={} counters=mouse_button:{} mouse_move:{} scroll:{} key:{}",
                            tab.id,
                    ...
                    other => {
                        eprintln!(
                            "[Girlbat] engine-thread-smoke input {label} reply failed: {other:?}"
                        );
                        ok = false;
                    }
                }
    ...
    if let Some(tab_id) = created_tab_id {
        ...
        while Instant::now() < snapshot_deadline {
            ...
            match snapshot_rx.recv_timeout(Duration::from_secs(5)) {
                Ok(snapshot) => {
                    eprintln!("[Girlbat] engine-thread-smoke snapshot={snapshot:?}");
                ...
                Err(error) => {
                    eprintln!("[Girlbat] engine-thread-smoke snapshot timed out: {error}");
                    ok = false;
                    break;
                }
            }
        ...
        if ok && !matched_snapshot {
            eprintln!("[Girlbat] engine-thread-smoke snapshot never matched expected state");
            ok = false;
        }
    }
        ...
        if ok {
            match navigate_rx.recv_timeout(Duration::from_secs(5)) {
                Ok(Ok(tab)) if tab.url == DIALOG_DATA_URL => {
                    eprintln!("[Girlbat] engine-thread-smoke dialog navigate tab_id={tab_id}");
                }
                other => {
                    eprintln!("[Girlbat] engine-thread-smoke dialog navigate failed: {other:?}");
                    ok = false;
                }
            }
        }
    ...
    if let Some(tab_id) = created_tab_id {
                    ...
                    if let Some(view) = snapshot.iter().find(|view| {
                        view.id == tab_id
                            ...
                            && view
                                ...
                                .contains("girlbat-dialog-message")
                            && view
                                .last_javascript_dialog_default_prompt_text
                                .contains("girlbat-dialog-default")
                            && !view.last_javascript_dialog_origin_url.is_empty()
                    }) {
                        dialog_request_id = view.last_javascript_dialog_request_id;
                        eprintln!(
                            "[Girlbat] engine-thread-smoke dialog request tab_id={tab_id} request_id={dialog_request_id}"
                        );
                        break;
                    }
                }
                Err(error) => {
                    eprintln!("[Girlbat] engine-thread-smoke dialog snapshot timed out: {error}");
                    ok = false;
                    break;
                }
            }
        ...
        if ok && dialog_request_id == 0 {
            eprintln!("[Girlbat] engine-thread-smoke dialog request never arrived");
            ok = false;
        }
        if ok {
            ...
            ok = command_tx
                .send(Command::JavaScriptDialogReply {
                    ...
                    accepted: true,
                    prompt_text: "girlbat-dialog-replied".to_string(),
                    reply: reply_tx,
                })
                .is_ok();
            if ok {
                match reply_rx.recv_timeout(Duration::from_secs(5)) {
                    ...
                    {
                        eprintln!(
                            "[Girlbat] engine-thread-smoke dialog reply tab_id={tab_id} request_id={dialog_request_id}"
                        );
                    }
                    other => {
                        eprintln!("[Girlbat] engine-thread-smoke dialog reply failed: {other:?}");
                        ok = false;
                    }
                }
            }
    ...
    if let Some(tab_id) = created_tab_id {
        match recv_event(&event_rx, "dialog load finished") {
            Some(Event::LoadFinished {
                ...
                eprintln!(
                    "[Girlbat] engine-thread-smoke dialog load finished tab_id={event_tab_id} url={url}"
                );
            }
            Some(Event::Failed { message }) => {
                eprintln!("[Girlbat] engine-thread-smoke dialog load failed: {message}");
                ok = false;
            }
            other => {
                eprintln!("[Girlbat] engine-thread-smoke unexpected dialog load event: {other:?}");
                ok = false;
            }
        }
    }
    ...
    let saw_shutdown = match recv_event(&event_rx, "shutdown") {
        Some(Event::ShutdownComplete) => {
            eprintln!("[Girlbat] engine-thread-smoke shutdown complete");
            true
        }
        other => {
            eprintln!("[Girlbat] engine-thread-smoke unexpected shutdown event: {other:?}");
            ok = false;
            false
        }
    };
    ...
    if saw_shutdown && join.join().is_err() {
        eprintln!("[Girlbat] engine-thread-smoke owner thread panicked");
        ok = false;
    }
...
fn recv_event(rx: &mpsc::Receiver<Event>, label: &str) -> Option<Event> {
    match rx.recv_timeout(Duration::from_secs(30)) {
        Ok(event) => Some(event),
        Err(error) => {
            eprintln!("[Girlbat] engine-thread-smoke timed out waiting for {label}: {error}");
            None
        }
    }
}
...
fn owner_thread(rx: mpsc::Receiver<Command>, events: mpsc::Sender<Event>) {
    ...
    loop {
        ...
        let command = if should_pump {
            ...
            if !idle_logged {
                eprintln!("[Girlbat] engine owner idle blocking on commands");
                idle_logged = true;
            }
            match rx.recv() {
...
fn process_command(
    ...
    match command {
        ...
        Command::CloseTab { tab_id, reply } => {
            ...
            if removed {
                eprintln!("[Girlbat] engine CloseTab destroyed view tab_id={tab_id}");
            }
...
fn create_tab(
    ...
    eprintln!(
        "[Girlbat] engine CreateTab created view tab_id={} pane_id={} url={} dark={} gui_active={}",
...
fn navigate_tab(
    ...
    eprintln!(
        "[Girlbat] engine Navigate requested tab_id={} url={}",
...
fn resize_tab(
    ...
    tab.height = height;
    eprintln!(
        "[Girlbat] engine Resize applied tab_id={} size={}x{}",
...
fn mouse_event(
    ...
    eprintln!(
        "[Girlbat] engine MouseEvent queued tab_id={} type={} button={} coords=({:.2}, {:.2}) click_count={} modifiers={} mouse_button_events={}",
...
fn mouse_move(
    ...
    eprintln!(
        "[Girlbat] engine MouseMove queued tab_id={} coords=({:.2}, {:.2}) modifiers={} mouse_move_events={}",
...
fn scroll_event(
    ...
    tab.scroll_events += 1;
    eprintln!(
        "[Girlbat] engine ScrollEvent queued tab_id={} coords=({:.2}, {:.2}) delta=({:.2}, {:.2}) phase={} momentum_phase={} precise={} modifiers={} scroll_events={}",
...
fn key_event(
    ...
    tab.key_events += 1;
    eprintln!(
        "[Girlbat] engine KeyEvent queued tab_id={} type={} windows_key_code={} utf8_len={} modifiers={} key_events={}",
        tab.id,
        event_type,
        windows_key_code,
        utf8.len(),
...
fn javascript_dialog_reply(
    ...
    match result {
        Ok(()) => {
            eprintln!(
                "[Girlbat] engine JavaScriptDialogReply applied tab_id={} request_id={} accepted={} prompt_text_len={}",
                tab.id,
                request_id,
                accepted,
                prompt_text.len()
        ...
        Err(error) => {
            eprintln!(
                "[Girlbat] engine JavaScriptDialogReply failed tab_id={} request_id={} accepted={} error={error}",
...
fn set_focus(
    ...
    tab.focused = focused;
    eprintln!(
        "[Girlbat] engine FocusChanged tracked tab_id={} focused={}",
        tab.id, tab.focused
    );
    Ok(snapshot_from_tab(tab))
}
...
fn set_color_scheme(
    ...
    tab.dark = dark;
    eprintln!(
        "[Girlbat] engine SetColorScheme applied tab_id={} dark={}",
        tab.id, tab.dark
    );
    Ok(snapshot_from_tab(tab))
}
...
fn set_gui_active(
    ...
    if tab_id == 0 {
        ...
        eprintln!(
            "[Girlbat] engine SetGuiActive applied tab_id=0 active={} target_count={}",
            active,
            snapshots.len()
        );
    ...
    tab.gui_active = active;
    eprintln!(
        "[Girlbat] engine SetGuiActive applied tab_id={} active={} target_count=1",
...
fn publish_title_changes(tabs: &mut HashMap<i64, OwnedTab>) {
    for tab in tabs.values_mut() {
        let title = match tab.view.take_title_changed() {
            ...
            Err(error) => {
                eprintln!(
                    "[Girlbat] engine title change poll failed tab_id={} error={error}",
                    tab.id
                );
                continue;
            }
    ...
    for tab in tabs.values_mut() {
        loop {
            let message = match tab.view.take_console_message() {
                ...
                Err(error) => {
                    eprintln!(
                        "[Girlbat] engine console message poll failed tab_id={} error={error}",
                        tab.id
                    );
                    break;
                }
    ...
    for tab in tabs.values_mut() {
        loop {
            let request = match tab.view.take_javascript_dialog_request() {
                ...
                Err(error) => {
                    eprintln!(
                        "[Girlbat] engine JavaScript dialog request poll failed tab_id={} error={error}",
                        tab.id
                    );
                    break;
                }
    ...
    for tab in tabs.values_mut() {
        loop {
            let cursor_type = match tab.view.take_cursor_changed() {
                ...
                Err(error) => {
                    eprintln!(
                        "[Girlbat] engine cursor change poll failed tab_id={} error={error}",
                        tab.id
                    );
                    break;
                }
        ...
        loop {
            let target_url = match tab.view.take_target_url_changed() {
                ...
                Err(error) => {
                    eprintln!(
                        "[Girlbat] engine target URL poll failed tab_id={} error={error}",
                        tab.id
                    );
                    break;
                }
    ...
    for tab in tabs.values_mut() {
        loop {
            let crash = match tab.view.take_renderer_crashed() {
                ...
                Err(error) => {
                    eprintln!(
                        "[Girlbat] engine renderer crash poll failed tab_id={} error={error}",
                        tab.id
                    );
                    break;
                }
...
fn publish_finished_loads(tabs: &mut HashMap<i64, OwnedTab>, events: &mpsc::Sender<Event>) {
    for (tab_id, tab) in tabs.iter_mut() {
        ...
        if tab.view.did_finish_load() {
            ...
            eprintln!(
                "[Girlbat] engine load finished tab_id={} url={}",
                tab.id, tab.last_url
            );
...
fn publish_renderer_crashed(tab_id: i64, crash: &AbiRendererCrash) {
    ...
    eprintln!(
        "[Girlbat] engine RendererCrashed sent_to={sent} tab_id={tab_id} status={} code={} url={} can_reload={} mode=crash-callback",
...
fn publish_cursor_changed(tab_id: i64, cursor_type: i32) {
    ...
    let sent = crate::ipc::send(&msg);
    eprintln!(
        "[Girlbat] engine CursorChanged sent_to={sent} tab_id={tab_id} cursor_type={cursor_type} mode=hover-callback"
    );
}
...
fn publish_target_url_changed(tab_id: i64, url: &str) {
    ...
    let sent = crate::ipc::send(&msg);
    eprintln!(
        "[Girlbat] engine TargetUrlChanged sent_to={sent} tab_id={tab_id} url={url} mode=hover-callback"
    );
}
...
fn publish_javascript_dialog_request(tab_id: i64, request: &AbiJavaScriptDialogRequest) {
    ...
    let sent = crate::ipc::send(&msg);
    eprintln!(
        "[Girlbat] engine JavaScriptDialogRequest sent_to={sent} tab_id={tab_id} request_id={} type={} origin={} message={} default={} mode=dialog-callback",
        request.request_id,
        request.dialog_type,
        request.origin_url,
        request.message,
...
fn publish_console_message(tab_id: i64, message: &AbiConsoleMessage) {
    ...
    let sent = crate::ipc::send(&msg);
    eprintln!(
        "[Girlbat] engine ConsoleMessage sent_to={sent} tab_id={tab_id} level={} line_no={} source={} message={} mode=console-callback",
...
fn publish_tab_ready(tab: &TabSnapshot) {
    ...
    let sent = crate::ipc::send(&msg);
    eprintln!(
        "[Girlbat] engine TabReady sent_to={} tab_id={} pane_id={}",
...
fn export_and_publish_render_surface(tab: &mut OwnedTab) -> bool {
    ...
    if attempt > RENDER_SURFACE_RETRY_LIMIT {
        eprintln!(
            "[Girlbat] engine render surface retry exhausted tab_id={} attempts={}",
    ...
    let exported = match tab.view.export_render_surface() {
        ...
        Err(error) => {
            eprintln!(
                "[Girlbat] engine render surface export failed tab_id={} error={error}",
                tab.id
            );
            return false;
        }
        ...
        || exported.pixel_height == 0
    {
        if should_log_retry || attempt == RENDER_SURFACE_RETRY_LIMIT {
            eprintln!(
                "[Girlbat] engine render surface export pending tab_id={} attempt={} has_surface={} surface_port={} pixel={}x{}",
                tab.id,
                attempt,
    ...
    let Some(metadata) = crate::render_channel::send_exported_surface_global(exported) else {
        if should_log_retry || attempt == RENDER_SURFACE_RETRY_LIMIT {
            eprintln!(
                "[Girlbat] engine render surface send skipped tab_id={} attempt={} has_surface={} surface_port={} pixel={}x{}",
                tab.id,
                attempt,
                exported.has_surface,
                exported.surface_port,
    ...
    eprintln!(
        "[Girlbat] engine RenderSurface metadata sent_to={sent} tab_id={} generation={} attachment_id={}",
...
fn publish_url_changed(tab_id: i64, url: &str) {
    ...
    let sent = crate::ipc::send(&msg);
    eprintln!("[Girlbat] engine UrlChanged sent_to={sent} tab_id={tab_id} url={url}");
}

fn publish_loading_state(tab_id: i64, state: &str, progress: u64) {
    let msg = TermSurfMessage {
    ...
    let sent = crate::ipc::send(&msg);
    eprintln!(
        "[Girlbat] engine LoadingState sent_to={sent} tab_id={tab_id} state={state} progress={progress}"
    );
}
...
fn publish_title_changed(tab_id: i64, title: &str) {
    ...
    let sent = crate::ipc::send(&msg);
    eprintln!(
        "[Girlbat] engine TitleChanged sent_to={sent} tab_id={tab_id} title={title} mode=title-callback"
    );
}
