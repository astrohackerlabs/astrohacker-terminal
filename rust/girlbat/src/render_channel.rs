...
fn connect_impl(service_name: &str) -> Option<RenderChannel> {
    let service = match CString::new(service_name) {
        Ok(value) => value,
        Err(_) => {
            eprintln!("[Girlbat] render side-channel skipped: service name contains NUL");
            return None;
        }
    };
    ...
    if result == TSRC_OK {
        eprintln!(
            "[Girlbat] render side-channel handshake sent service={service_name} receive_port={receive_port}"
        );
        ...
        eprintln!(
            "[Girlbat] render side-channel handshake failed service={service_name} result={}",
            result_name(result)
        );
        None
    }
...
impl RenderChannel {
    ...
    fn wait_for_surface_receiver(&mut self) -> bool {
        ...
        if result != TSRC_OK {
            eprintln!(
                "[Girlbat] render side-channel receiver wait failed result={}",
                result_name(result)
            );
            return false;
        }
        ...
        eprintln!(
            "[Girlbat] render side-channel surface receiver ready send_port={surface_send_port}"
        );
        true
    }
    ...
    pub fn send_exported_surface(
        ...
        if self.surface_send_port == 0 {
            eprintln!("[Girlbat] render side-channel has no surface receiver send port");
            deallocate_exported_surface_port(exported);
            return None;
        }
        if !usable_export(exported) {
            eprintln!(
                "[Girlbat] render side-channel export is not transferable export={exported:?}"
            );
            deallocate_exported_surface_port(exported);
            return None;
        }
        ...
        if result == TSRC_OK {
            eprintln!(
                "[Girlbat] render side-channel real surface sent width={} height={} generation={} attachment_id={attachment_id}",
            ...
            eprintln!(
                "[Girlbat] render side-channel real surface failed result={}",
                result_name(result)
            );
            None
        }
...
#[cfg(not(target_os = "macos"))]
fn connect_impl(service_name: &str) -> Option<RenderChannel> {
    eprintln!("[Girlbat] render side-channel unsupported on this platform service={service_name}");
    None
}
...
pub fn real_frame_attachment_smoke(exported: RenderSurfaceExport) -> bool {
    if !usable_export(exported) {
        eprintln!("[Girlbat] real-frame-attachment-smoke invalid export={exported:?}");
        deallocate_exported_surface_port(exported);
        return false;
    }
    ...
    let service = match CString::new(service_name.clone()) {
        Ok(service) => service,
        Err(_) => {
            eprintln!("[Girlbat] real-frame-attachment-smoke service name contains NUL");
            deallocate_exported_surface_port(exported);
            return false;
        }
    };
    ...
    if result != TSRC_OK {
        eprintln!(
            "[Girlbat] real-frame-attachment-smoke register failed result={}",
            result_name(result)
        );
        deallocate_exported_surface_port(exported);
        return false;
    ...
    if result != TSRC_OK {
        eprintln!(
            "[Girlbat] real-frame-attachment-smoke child connect failed result={}",
            result_name(result)
        );
        deallocate_exported_surface_port(exported);
        let _ = receiver.join();
    ...
    if result != TSRC_OK {
        eprintln!(
            "[Girlbat] real-frame-attachment-smoke surface receiver wait failed result={}",
            result_name(result)
        );
        deallocate_exported_surface_port(exported);
    ...
    if result != TSRC_OK {
        eprintln!(
            "[Girlbat] real-frame-attachment-smoke surface send failed result={}",
            result_name(result)
        );
        let _ = receiver.join();
        return false;
    ...
    let metadata = match receiver.join() {
        ...
        Err(_) => {
            eprintln!("[Girlbat] real-frame-attachment-smoke receiver thread panicked");
            return false;
        }
    };
    ...
    eprintln!("[Girlbat] real-frame-attachment-smoke imported metadata={metadata:?} matched={ok}");
    if ok {
        eprintln!("[Girlbat] real-frame-attachment-smoke PASS attachment_id={attachment_id}");
    }
    ok
}
...
pub fn real_frame_attachment_smoke(_exported: RenderSurfaceExport) -> bool {
    eprintln!("[Girlbat] real-frame-attachment-smoke unsupported on this platform");
    false
}
...
fn receive_one_surface(control_port: TsrcPort) -> Option<TsrcSurfaceMetadata> {
    ...
    if result != TSRC_OK {
        eprintln!(
            "[Girlbat] real-frame-attachment-smoke receiver child wait failed result={}",
            result_name(result)
        );
        return None;
    }
    ...
    if result != TSRC_OK {
        eprintln!(
            "[Girlbat] real-frame-attachment-smoke send receiver failed result={}",
            result_name(result)
        );
        return None;
    }
    ...
    if result != TSRC_OK {
        eprintln!(
            "[Girlbat] real-frame-attachment-smoke receive surface failed result={}",
            result_name(result)
        );
        return None;
    }
