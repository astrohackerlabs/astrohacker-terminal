...
pub fn connect(path: &str) -> Option<UnixStream> {
    let stream = match UnixStream::connect(path) {
        Ok(stream) => stream,
        Err(error) => {
            eprintln!("[Girlbat] connect failed: {error}");
            return None;
        }
    };
...
pub fn listen(path: &str, shutdown: Sender<String>) -> io::Result<()> {
    ...
    eprintln!("[Girlbat] listener bound: {path}");

    std::thread::spawn(move || {
        for conn in listener.incoming() {
            match conn {
                Ok(stream) => {
                    eprintln!("[Girlbat] listener client connected");
                    let reader = match stream.try_clone() {
                        Ok(reader) => reader,
                        Err(error) => {
                            eprintln!("[Girlbat] listener clone failed: {error}");
                            continue;
                        }
                    };
                    WRITERS.lock().unwrap().push(stream);
                ...
                Err(error) => {
                    eprintln!("[Girlbat] listener accept error: {error}");
...
fn read_messages(mut stream: UnixStream, quit_on_eof: bool, shutdown: Sender<String>) {
    ...
    loop {
        let n = match stream.read(&mut tmp) {
            ...
            Err(error) => {
                eprintln!("[Girlbat] socket read error: {error}");
                if quit_on_eof {
        ...
        while buf.len() >= 4 {
            ...
            match TermSurfMessage::decode(payload) {
                ...
                Err(error) => eprintln!("[Girlbat] protobuf decode error: {error}"),
            }
            buf.drain(..4 + msg_len);
        }
