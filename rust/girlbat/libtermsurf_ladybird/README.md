...
This directory contains the C ABI wrapper that lets the Rust `girlbat`
...
- `stub` (default): a standalone C stub that exports stable `ts_ladybird_*`
  symbols and keeps normal Girlbat builds cheap.
- `real`: a Ladybird-built C++ dylib that runs the first headless
  WebContent-backed lifecycle probe behind the C ABI.

## Intended Boundary

Later experiments should add a C ABI surface similar to the Roamium and Surfari
wrappers:
...
- PDF capability/runtime asset checks. See
  [Girlbat PDF support](../../docs/girlbat-pdf-support.md).
...
```bash
girlbat/libtermsurf_ladybird/build.sh
```

The output dylib is generated under `girlbat/libtermsurf_ladybird/build/`, which
is ignored by git.
...
```bash
TERMSURF_LADYBIRD_BACKEND=real rust/girlbat/libtermsurf_ladybird/build.sh
```

In real mode, `build.sh --clean` only removes the staged
`girlbat/libtermsurf_ladybird/build/` output. It does not remove
...
- one runtime handle per process, matching TermSurf's one-profile-per-process
  model;
...
callback, and destroys the handles. It is not visible rendering or TermSurf
protocol parity.

## Render-Surface Probe

Issue 884 Experiment 12 adds `ts_ladybird_view_render_surface_probe`. In stub
...
TermSurf-owned `HeadlessWebView` subclass to reach the protected backing-store
state that Ladybird's AppKit bridge also uses. The probe reports whether the
current view has a `Gfx::SharedImageBuffer`, the pixel dimensions of that
buffer, whether `on_ready_to_paint` has fired, whether
...
1. Prove Ladybird build/link consumption with a `HeadlessWebView` probe. Done in
   Issue 884 Experiment 5 with the in-tree `TestGirlbatHeadlessLifecycle`
   target.
2. Prove a live WebContent-backed `HeadlessWebView` under `Core::EventLoop`.
   Done in Issue 884 Experiment 5.
3. Prove deterministic headless navigation callbacks. Done in Issue 884
   Experiment 5 with a `data:` URL load-finish callback.
4. Move that deterministic lifecycle path behind the C ABI. Done in Issue 884
...
5. Convert the one-shot lifecycle proof into persistent runtime and view
   handles. Done in Issue 884 Experiment 7.
6. Design a TermSurf-specific `ViewImplementation` subclass that can expose a
   presentable backing store or IOSurface for Ghostboard. Done in Issue 884
   Experiment 12 through ABI reachability only.
...
presentation; TermSurf will need an equivalent presentable surface boundary.
