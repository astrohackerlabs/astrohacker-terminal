...
if ! test -d "$WEBKIT_BUILD/WebKit.framework"; then
  echo "POINTER_SMOKE_FAIL missing_webkit_framework path=$WEBKIT_BUILD/WebKit.framework" >&2
  exit 1
fi

TERMSURF_WEBKIT_CONFIGURATION="$CONFIGURATION" "$LIB_ROOT/build.sh"
FIXTURE="file://$REPO_ROOT/rust/test-content/pointer-stream.html"
DYLD_FRAMEWORK_PATH="$WEBKIT_BUILD${DYLD_FRAMEWORK_PATH:+:$DYLD_FRAMEWORK_PATH}" \
  "$LIB_ROOT/build/pointer-stream-smoke" "$FIXTURE?peer=A" "$FIXTURE?peer=B"
