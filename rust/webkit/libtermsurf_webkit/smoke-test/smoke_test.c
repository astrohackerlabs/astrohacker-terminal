...
static void query_focus_state(void *user_data)
{
    ...
    ts_webkit_test_evaluate_javascript(
        state->web_contents,
        "JSON.stringify({ focus: window.__webkitState.focus, focusIn: window.__webkitState.focusIn, hasFocus: document.hasFocus(), activeElement: document.activeElement ? document.activeElement.id : \"\" })",
        check_focus_result,
        state);
}
