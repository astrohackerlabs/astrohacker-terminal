...
LOG_DIR="$ROOT/logs/issue-867-exp2-installed-runtime"
...
ROAMIUM="${TERMSURF_ROAMIUM:-/opt/homebrew/opt/astrohacker-terminal-roamium/roamium}"
SURFARI="${TERMSURF_SURFARI:-/opt/homebrew/opt/astrohacker-terminal-surfari/surfari}"
SURFARI_PREFIX="${TERMSURF_SURFARI_PREFIX:-/opt/homebrew/opt/astrohacker-terminal-surfari}"
...
fixture_title() {
  case "$1" in
    roamium) printf 'Issue 867 Roamium Fixture\n' ;;
    surfari) printf 'Issue 867 Surfari Fixture\n' ;;
...
<!doctype html>
<meta charset="utf-8">
<title>Issue 867 Roamium Fixture</title>
<h1>ISSUE867_ROAMIUM_READY</h1>
EOF
  cat >"$SITE_DIR/surfari.html" <<'EOF'
<!doctype html>
<meta charset="utf-8">
<title>Issue 867 Surfari Fixture</title>
