...
LOG_DIR="$ROOT/logs/issue-870-exp1-surfari-cursor-hover"
RUN_DIR="$(mktemp -d "${TMPDIR:-/tmp}/termsurf-issue870-exp1.XXXXXX")"
APP="${TERMSURF_GHOSTBOARD_APP:-$ROOT/ghostboard/macos/build/Release/Astrohacker Terminal.app}"
APP_BIN="$APP/Contents/MacOS/aht"
WEB="${TERMSURF_WEB:-$ROOT/target/release/web}"
SURFARI="${TERMSURF_SURFARI:-$ROOT/target/release/ah-webkitd}"
ROAMIUM="${TERMSURF_ROAMIUM:-$ROOT/chromium/src/out/Default/ah-chromiumd}"
WEBKIT_RELEASE="${TERMSURF_WEBKIT_RELEASE:-$ROOT/webkit/src/WebKitBuild/Release}"
...
<html>
  <head>
    <meta charset="utf-8">
    <title>Issue 870 Cursor Fixture</title>
    <style>
      html,
      body {
        margin: 0;
  ...
  <body>
    ...
    <a id="link" href="https://example.com/">Issue 870 test link</a>
...
if [ "$BROWSER" = "surfari" ]; then
  post_move_pair_until "$LINK_X" "$LINK_Y" "$LINK2_X" "$LINK2_Y" "$APP_LOG" "$LINK_START" "surfari-cursor phase=dom tab=${BROWSER_TAB_ID} raw=-1 mapped=2 suppress=0" "Surfari derived hand cursor without suppression"
else
...
if [ "$BROWSER" = "surfari" ]; then
  if tail -n +1 "$APP_LOG" | grep -F "surfari-cursor phase=suppressed" >/dev/null 2>&1; then
