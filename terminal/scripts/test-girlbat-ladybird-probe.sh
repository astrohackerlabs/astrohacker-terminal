...
PROBE_BUILD_DIR="$REPO_DIR/build/girlbat-headless-lifecycle-probe"
...
if [ -f "$IN_TREE_PROBE" ]; then
  TARGET="TestGirlbatHeadlessLifecycle"
  ...
  if grep -q "PASS: Girlbat headless lifecycle probe reached Tier 3" "$LOG_DIR/direct-run.stdout.log"; then
    record "result=tier3"
...
record "probe_build=cmake --build $PROBE_BUILD_DIR --target girlbat_headless_lifecycle_probe"
if ! cmake --build "$PROBE_BUILD_DIR" --target girlbat_headless_lifecycle_probe \
...
record "result=tier1"
record "probe_binary=$PROBE_BUILD_DIR/girlbat_headless_lifecycle_probe"
