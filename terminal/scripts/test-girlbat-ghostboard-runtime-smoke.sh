...
GIRLBAT="${TERMSURF_GIRLBAT:-$ROOT/target/debug/girlbat}"
...
wait_for_pattern "TermSurf message decoded type=HelloRequest" "WebTUI connected to Ghostboard"
wait_for_pattern "SetOverlay: pane_id=.* profile=default browser=girlbat url=${URL}" "SetOverlay names Girlbat"
wait_for_literal "SetOverlay: named browser resolved browser=girlbat env=TERMSURF_GIRLBAT_PATH path=${GIRLBAT}" "Ghostboard resolved named Girlbat"
wait_for_pattern "spawned browser path=${GIRLBAT} pid=[0-9]+ profile=default browser=girlbat .*render_surface_service=com\\.termsurf\\.girlbat\\.render\\." "Ghostboard spawned Girlbat with render side-channel"
wait_for_literal "[Girlbat] render side-channel global connected=true" "Girlbat connected render side-channel"
wait_for_pattern "ServerRegister: profile=default browser=girlbat" "Ghostboard registered Girlbat"
wait_for_pattern "TabReady: pane_id=.* tab_id=[0-9]+" "Ghostboard mapped Girlbat TabReady"
wait_for_pattern "BrowserReady: pane_id=.* tab_id=[0-9]+ socket=.* browser=girlbat" "Ghostboard sent BrowserReady for Girlbat"
...
wait_for_pattern "RenderSurface: tab_id=[0-9]+ pane_id=.* generation=[0-9]+ pixel=[1-9][0-9]*x[1-9][0-9]* .* attachment_id=[1-9][0-9]*" "Ghostboard received matched Girlbat RenderSurface"
...
log "PASS: girlbat Ghostboard runtime structural presentation smoke"
