...
The current headless Ladybird path cannot reach that surface through a public
API:
...
Ghostboard, Chromium, and WebKit. An IOSurface/Mach-port render surface has
...
Not the next step. Astrohacker Terminal's engine architecture is one browser
profile server process speaking protobuf/Unix sockets to Ghostboard. Embedding
a Ladybird `NSView` directly into Ghostboard would cross that process boundary
in the wrong direction and would not generalize cleanly to other terminal
frontends.

### New IOSurface/Mach-Port Render Surface Message

Chosen direction. Ladybird should expose a render surface compatible with
...
This keeps `CaContext` semantics stable for Chromium and WebKit while allowing
Ladybird to use Ladybird's native rendering primitive honestly.
...
2. Ladybird-owned subclass: `libtermsurf_ladybird` can define a TermSurf protocol-specific
...
TermSurf protocol-specific `ViewImplementation` subclass inside
...
The design must preserve existing Chromium and WebKit behavior. Those engines
...
- keep the existing CAContext path unchanged for Chromium and WebKit.
...
This confirms outcome 2 from the reachability decision: a Ladybird-owned subclass
...
token only to spawned Ladybird processes, and Ladybird parses that token without
...
Ladybird-only side-channel service, Ladybird sends back a render-channel port, and
...
Ghostboard sends Ladybird a Ghostboard-owned surface receive port over the
Experiment 17 child port, and Ladybird sends a deterministic test IOSurface Mach
...
- Ghostboard routes and stores the metadata for Ladybird panes while leaving
  Chromium/WebKit `CaContext` behavior unchanged.
...
1. The Ladybird-backed ABI exports a real IOSurface Mach send right from
   `TermSurfWebView`.
2. Ladybird carries that send right over the render side channel with a nonzero
   `attachment_id`.
3. Ghostboard accepts nonzero `RenderSurface` metadata only when it has imported
   the matching side-channel attachment.
4. The existing length-prefixed protobuf stream and Chromium/WebKit `CaContext`
   behavior remain unchanged.

Experiment 22 adds headless viewport resize control through the Ladybird C ABI.
Ladybird now routes `Resize` to `ts_ladybird_view_resize`, which calls
...
coverage matrix. For Ladybird, emitting `CaContext` would be a protocol
regression because Ladybird uses the `RenderSurface` metadata message plus the
IOSurface/Mach-port side channel. Chromium and WebKit keep using the existing
`CaContext` path.
...
3. Swift retains the opaque `IOSurfaceRef` before async main-queue dispatch and
   ...
   Chromium/WebKit `CALayerHost` path.
...
normal runtime route as well: WebTUI runs `web --browser ladybird` inside Debug
Ghostboard, Ghostboard resolves the named Ladybird executable through
`ASTROHACKER_LADYBIRD_PATH`, Ladybird loads a normal local HTTP page, emits matched
