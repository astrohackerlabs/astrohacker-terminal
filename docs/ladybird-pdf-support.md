# Ladybird PDF Support

Ladybird uses Ladybird's built-in PDF path. Current Ladybird source routes
...
Ladybird's Cargo build stages pdf.js assets into the local runtime resource root
...
The smoke test asks Ladybird for its resolved runtime root with
...
```bash
scripts/test-ladybird-pdf-smoke.py
```

The smoke test proves:

- Ladybird reports `navigator.pdfViewerEnabled === true` through the
  `ConsoleMessage` protocol path.
- Ladybird exposes `application/pdf` and `text/pdf` through
  `navigator.mimeTypes`.
- Standard PDF plugin names are present when the Ladybird PDF flag is true.
- The runtime pdf.js assets are present in the resource root Ladybird reports.
- A local one-page PDF served as `application/pdf` is requested by Ladybird.
- No `RendererCrashed` event is observed during the PDF navigation window.
- Ladybird remains queryable after the PDF navigation attempt.

## Limitations

This is smoke evidence, not full PDF parity with Chromium or WebKit.
