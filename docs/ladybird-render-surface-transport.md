...
```bash
scripts/audit-ladybird-render-transport.py
```
...
- `ladybird/src/ipc.rs`, `chromium/src/ipc.rs`, and `webkit/src/ipc.rs` write a
...
- `proto/termsurf.proto` also has metadata-only
  `RenderSurface render_surface = 43` for the Ladybird side-channel path.
...
- `ghostboard/pkg/macos/iosurface/iosurface.zig` can create and inspect
  ...
  `IOSurfaceLookupFromMachPort`.
- No TermSurf protocol receiver maps a Ladybird render-surface event to this
  IOSurface presentation layer.

## IOSurface ID Probe

The macOS SDK exposes `IOSurfaceID`, `IOSurfaceGetID`, and `IOSurfaceLookup`,
but that is not enough for Ladybird. The SDK also documents `kIOSurfaceIsGlobal`
...
Chromium and WebKit. Ladybird's Ladybird surface is an IOSurface/Mach-port
...
that proves how Ladybird can transfer the IOSurface Mach send right to
...
Ladybird must keep the exported surface alive until Ghostboard has imported or
...
a per-browser-process render side channel when spawning Ladybird, pass the
...
- Chromium and WebKit do not receive the argument and continue using the
  existing `CaContext` rendering path.
- Ladybird parses the argument and records whether the render side-channel is
...
Ladybird rendering as visually correct on screen.

Experiment 22 adds a related control-path proof, not a transport proof. Ladybird
...
Experiment 24 records that Ladybird's non-use of legacy `CaContext` is
intentional. `RenderSurface` remains the authoritative Ladybird metadata path,
...
- `render-channel/termsurf_render_channel.h` and
  ...
  bootstrap handshake.
- Ghostboard links that shim and calls `tsrc_register_service()` for Ladybird
  only, before spawning the browser process.
- Ladybird links the same shim and calls `tsrc_child_connect_and_send()` when
...
- Ghostboard receives the child render-channel send right on a detached bounded
  ...
  discarded rights.
- Chromium and WebKit stay on the existing `CaContext` path and do not register
...
app smoke test should confirm the same mechanism from the signed
`Astrohacker Terminal.app` runtime before relying on it for user-visible
rendering.
...
- Ghostboard then allocates a new Ghostboard-owned surface receive port, inserts
  a send right, and sends that send right to Ladybird using
  `TSRC_SURFACE_RECEIVER_MESSAGE_ID`;
- Ladybird receives that Ghostboard surface send right, creates a deterministic
...
The directionality was correct for rendering: Ladybird could deliver IOSurface
